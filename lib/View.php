...
class Q_View extends Smarty{
	...
	function __construct(){
		$this->template_dir = Q_TPL_DIR;
		$this->compile_dir = Q_TMP_DIR.'/templates';
		$this->register_function('formhash', array('Q_View','formhash'));
		$this->register_function('show_pages', array('Q_View','show_pages'));
		$this->register_function('get_url', array('Q_View','get_url'));
		$this->assign_by_ref('param',Q_Request::$_params);
	}
	...
	function trigger_error($error_msg, $error_code = 500){
		throw new Q_Exception($error_msg, $error_code);
	}
